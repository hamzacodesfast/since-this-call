...
async function fixCZAster() {
    console.log('🔶 Repairing CZ Binance Aster Data...');
    ...
    try {
        // 1. Re-analyze with new logic (Aster -> ASTER)
        console.log(`Re-analyzing ${tweetId}...`);
        const result = await analyzeTweet(tweetId);
        console.log(`New Symbol: ${result.analysis.symbol} (Type: ${result.analysis.type}, Sentiment: ${result.analysis.sentiment})`);
        ...
        console.log('Updating Profile & History...');
