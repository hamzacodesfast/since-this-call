...
const KNOWN_AFFECTED = [
    'altcoindaily', 'kkernttb', 'nextbigtrade', 'nehalzzzz1', 'capexandchill',
    'echoanalysis', 'midascabal', 'stefan_b_trades', 'satoshiflipper',
    'dannycheng2022', 'spxsatoshi', 'themoonhailey', 'stcquantagent',
    'shiftercrypto', '0xchainlog', 'layeryapper'
];
...
async function main() {
    ...
    for (const username of KNOWN_AFFECTED) {
        ...
        if (!historyData || historyData.length === 0) { console.log(`@${username}: no history, skipping`); continue; }
    ...
    console.log(`\nDone! Fixed ${fixedCount} profiles.`);
