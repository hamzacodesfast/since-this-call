...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const safeText = data.tweet.text.replace(/[^\x20-\x7E]/g, '');
        const safeAuthor = data.tweet.author.replace(/[^\x20-\x7E]/g, '');

        console.log('[API/OG] Data:', JSON.stringify(data, null, 2));
                    ...
                    >
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', marginBottom: 40 }}>
                            <div style={{ display: 'flex', flexDirection: 'column' }}>
                                <div style={{ fontSize: 32, fontWeight: 700 }}>{safeAuthor}</div>
                        ...
                        <div style={{ fontSize: 28, lineHeight: 1.4, color: '#e5e7eb', marginBottom: 40, height: 120, overflow: 'hidden' }}>
                            {`"${safeText.length > 130 ? safeText.slice(0, 130) + '...' : safeText}"`}
                        </div>
        ...
        console.error('[API/OG] Image Generation Error:', e);
