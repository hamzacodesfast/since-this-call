...
export function getRedisClient() {
    ...
    // Check if we are running locally (Standard Redis URL or localhost)
    if (url.includes('localhost') || url.includes('127.0.0.1') || url.startsWith('redis://')) {
        return new LocalRedisWrapper(url) as unknown as Redis;
    }
