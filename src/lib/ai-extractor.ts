...
export async function extractCallFromText(
    ...
    try {
        ...
        * Use these reference prices to determine direction if the tweet target is ambiguous. E.g. "Target 60k" for BTC is a CRASH prediction (SELL) if current is ${btcPrice.toLocaleString()}.
        ...
        - COMMUNITY DEFENSE: Phrases like "REAL community", "DIAMOND holders", "Organic movement", or "Community-led" are BULLISH (BUY) signals for the token, especially when uses to defend the token against FUD, teams, or scams.
        - CHART PRAISE / STRUCTURAL BULLISHNESS (CRITICAL OVERRIDE): If a user calls the chart "The best chart", "Beautiful", "Textbook", "God tier", or "A+ setup", the Action is BULLISH (BUY), even if they mention waiting for a dip. The core thesis is strength.
        - BUYING STRATEGY (ACCUMULATION): If a user explicitly says "Accumulating on dips", "Buying support", or "Bidding lower", AND praises the asset ("Best chart"), the Action is BULLISH (BUY). Do NOT mark valid accumulation plans as SELL just because the entry is lower.
